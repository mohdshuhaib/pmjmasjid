...
export default function SettingsPage() {
  const [activeTab, setActiveTab] = useState<"committee" | "security" | "rollover">("committee");
  ...
  const handleSaveMember = async (e: React.FormEvent) => {
    ...
    if (isUpdate) {
      const { error } = await supabase.from('committee_members')
        .update({ name: currentMember.name, role_key: currentMember.role_key, contact_number: currentMember.contact_number, display_order: currentMember.display_order })
        .eq('id', currentMember.id);
      if (error) showMessage('error', error.message);
      else showMessage('success', 'Committee member updated successfully!');
    } else {
      const { error } = await supabase.from('committee_members')
        .insert([{ name: currentMember.name, role_key: currentMember.role_key, contact_number: currentMember.contact_number, display_order: currentMember.display_order }]);
      if (error) showMessage('error', error.message);
  ...
  const cancelEdit = () => { setCurrentMember({ name: "", role_key: "", contact_number: "", display_order: 0 }); setIsEditing(false); };

  // --- PASSWORD ACTIONS ---
  const handlePasswordChange = async (e: React.FormEvent) => {
    e.preventDefault();
    if (passwords.newPassword !== passwords.confirmPassword) return showMessage('error', "Passwords do not match!");
    if (passwords.newPassword.length < 6) return showMessage('error', "Password must be at least 6 characters long.");

    setLoadingPassword(true);
    const { error } = await supabase.auth.updateUser({ password: passwords.newPassword });

    if (error) showMessage('error', error.message);
    else { showMessage('success', 'Password updated successfully!'); setPasswords({ newPassword: "", confirmPassword: "" }); }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-12">
      <div>
        ...
        <p className="text-slate-500 mt-1">Manage committee members, security, and automated tasks.</p>
      </div>
      ...
      <div className="flex overflow-x-auto gap-4 border-b border-slate-200">
        <button
          ...
          className={`pb-3 px-4 font-medium transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === 'committee' ? 'text-emerald-600 border-b-2 border-emerald-600' : 'text-slate-500 hover:text-slate-800'}`}
        >
          <Users className="w-4 h-4" /> Committee
        </button>
        <button
          onClick={() => setActiveTab("security")}
          className={`pb-3 px-4 font-medium transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === 'security' ? 'text-emerald-600 border-b-2 border-emerald-600' : 'text-slate-500 hover:text-slate-800'}`}
        >
          <Shield className="w-4 h-4" /> Account Security
        </button>
        <button
          onClick={() => setActiveTab("rollover")}
          className={`pb-3 px-4 font-medium transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === 'rollover' ? 'text-emerald-600 border-b-2 border-emerald-600' : 'text-slate-500 hover:text-slate-800'}`}
        >
          <DatabaseBackup className="w-4 h-4" /> Yearly Update
        </button>
      </div>
                    ...
                    <div key={member.id} className="p-4 flex justify-between items-center hover:bg-slate-50 transition-colors">
                      <div>
                        ...
                        <div className="flex items-center gap-3 text-sm mt-1">
                          <span className="bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded font-medium uppercase tracking-wider text-[10px]">{member.role_key}</span>
        ...
        <div className="max-w-md bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
          ...
          <form onSubmit={handlePasswordChange} className="space-y-4">
            <div>
              ...
              <input type="password" required minLength={6} value={passwords.newPassword} onChange={e => setPasswords({...passwords, newPassword: e.target.value})} className="w-full border border-slate-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-emerald-500" />
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1">Confirm New Password</label>
              <input type="password" required minLength={6} value={passwords.confirmPassword} onChange={e => setPasswords({...passwords, confirmPassword: e.target.value})} className="w-full border border-slate-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-emerald-500" />
            </div>
            <button type="submit" disabled={loadingPassword} className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 rounded-xl shadow-md transition-all disabled:opacity-50 mt-2">
