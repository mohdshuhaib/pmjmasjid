    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          ...
          <button
            onClick={onClose}
            className="p-2 bg-white rounded-full text-slate-400 hover:text-slate-700 shadow-sm border border-slate-200"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          {error && (
            <div className="p-3 mb-6 bg-red-50 text-red-700 rounded-xl text-sm border border-red-200">
              {error}
            </div>
          )}
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    readOnly
                    value={formData.pmj_no || "N/A"}
                    className="w-full bg-amber-100/50 border border-amber-200 rounded-lg p-2.5 text-amber-900 cursor-not-allowed outline-none font-mono"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-amber-700 uppercase tracking-wider mb-1">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    readOnly
                    value={formData.head_pmj_no || "N/A"}
                    className="w-full bg-amber-100/50 border border-amber-200 rounded-lg p-2.5 text-amber-900 cursor-not-allowed outline-none font-mono"
                  />
                </div>
              </div>
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
          ...
          <button
            ...
            disabled={loading}
            className="px-8 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow-md transition-all flex items-center gap-2 disabled:opacity-50"
          >
